...
function useCurrentMarker(setXY: Function, setCurrentBtnOpen: Function) {
  const _map = useSelector((state: RootState) => state.setMap._map);
  const [currentMarker, setCurrentMarker] = useState<any>(); //현재 위치
  ...
  useEffect(() => {
    ...
    if (navigator.geolocation) {
      ...
      navigator.geolocation.getCurrentPosition(function (position) {
        ...
        function displayCurrentMarker(locPosition: object) {
          // 마커를 생성합니다
