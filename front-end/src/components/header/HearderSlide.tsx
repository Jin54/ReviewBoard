...
const HeaderSlide = () => {
  ...
  const SetOpenMobileMenu = useCallback(
    (bool: boolean) => dispatch(setOpenMobileMenu(bool)),
    [dispatch]
  );
  ...
  const onClickURL = useCallback(
    (name: string) => dispatch(changeURL(name)),
    [dispatch]
  );
  const openLogin = useSelector((state: RootState) => state.openBool.login);
  const userName = useSelector((state: RootState) => state.userData.name);
...
const MBtn = styled.div`
  ...
  cursor: pointer;
`;
const MFoodBtn = styled(MBtn)<MFoodBtnType>`
  padding-top: 30px;
  padding-bottom: 30px;
...
`;
interface MFoodBtnType {
  selected: boolean;
}

const MHospitalBtn = styled(MBtn)<MHospitalBtnType>`
  padding-top: 15px;
  padding-bottom: 30px;
  font-weight: ${(props) => (props.selected ? "700" : "400")};
  @media screen and (max-width: 1000px) {
